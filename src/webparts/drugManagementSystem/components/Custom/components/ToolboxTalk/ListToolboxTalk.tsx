...
// import { _siteDataUtil, canShowSiteTabButtons, deleteToolboxTalkDetails, deleteToolboxTalkMaster, generateExcelTable, getCAMLQueryFilterExpression, logGenerator, mapSingleValue, onBreadcrumbItemClicked, UserActivityLog } from "../../../../../../Common/Util";
...
// import { PendingSignatureEmailModal } from "../../../../../../Common/PendingSignatureEmailModal";
...
//     const [isPendingEmailOpen, setIsPendingEmailOpen] = React.useState(false);
//     const [selectedMeeting, setSelectedMeeting] = React.useState<any[]>([]);
...
//     React.useEffect(() => {
//         const permission = canShowSiteTabButtons(selectedZoneDetails?.defaultSelectedSitesId || [], currentUserRoleDetail);
...
//             }
//             // setIsLoading(false);
//             // setSummaryData(getSummaryData(filteredList));
//             setFilteredData(filteredList);
//         };
//         // setIsLoading(true);
//         filterList();
//     }, [ToolboxTalkDetailsData, filterType, selectedStateId]);
//     const handleSendPendingEmail = () => {

//         if (!selectedAttendees?.value) return;

//         let dataToFilter: any[] = [];
        
//         if (!!UpdateItem) {
//             // If items are selected from grid, use only those selected items
//             dataToFilter = Array.isArray(UpdateItem) ? UpdateItem : [UpdateItem];
//         } else {
//             // Otherwise use all filtered data
//             dataToFilter = FilteredData;
//         }

//         const filteredMeetings = dataToFilter.filter((item: any) =>
//             item.pendingUserId?.includes(selectedAttendees.value)
//         );

//         if (filteredMeetings.length === 0) return;

//         setSelectedMeeting(filteredMeetings); // pass ARRAY
//         setIsPendingEmailOpen(true);
//     };
...
//         {isPendingEmailOpen && (
//             <PendingSignatureEmailModal
//                 isOpen={isPendingEmailOpen}
//                 onClose={() => setIsPendingEmailOpen(false)}
//                 Meetings={selectedMeeting}
//                 selectedAttendee={selectedAttendees}
//                 pageType={"ToolboxTalk"}
//             />
//         )}
//         {(!isSiteLevelComponent) && stateTabData.length > 0 && <div className="ms-Grid-row pt15">
...
//                     CustomselectionMode={
//                         ( // !!props.siteMasterId &&
//                             (!!PermissionArray && PermissionArray?.includes('Quaysafe') || currentUserRoleDetail.isAdmin || currentUserRoleDetail.isStateManager || currentUserRoleDetail?.siteManagerItem.filter((r: any) => r.Id == props.siteMasterId && r.SiteManagerId?.indexOf(currentUserRoleDetail.Id) > -1).length > 0))
...
//                         </>}
//                         <Link
//                             className={`actionBtn iconSize btnDanger ml-10 ${!selectedAttendees?.value ? "disabledIcon" : ""}`}
//                             onClick={selectedAttendees?.value ? handleSendPendingEmail : undefined}
//                             style={{
//                                 pointerEvents: selectedAttendees?.value ? "auto" : "none",
//                                 opacity: selectedAttendees?.value ? 1 : 0.4,
//                                 cursor: selectedAttendees?.value ? "pointer" : "not-allowed"
//                             }}
//                         >
//                             <TooltipHost
//                                 content={
//                                     selectedAttendees?.value
//                                         ? "Send Email Pending Attendees Signature"
//                                         : "Please select attendee first"
//                                 }
//                             >
//                                 <FontAwesomeIcon icon={"envelope"} className="actionIcon" />
//                             </TooltipHost>
//                         </Link>
...
//                         {(currentUserRoleDetail.isAdmin || currentUserRoleDetail?.siteManagerItem.filter((r: any) => r.Id == props.siteMasterId && r.SiteManagerId?.indexOf(currentUserRoleDetail.Id) > -1).length > 0 || currentUserRoleDetail.isStateManager) && <PrimaryButton text="Add" className="btn btn-primary margin-sm-add"
//                             onClick={() => {
//                                 let breadCrumItems: IBreadCrum[] = props.breadCrumItems;
//                                 breadCrumItems.push({ text: "Add Form", key: 'Add Form', currentCompomnetName: ComponentNameEnum.AddToolboxTalk, onClick: onBreadcrumbItemClicked, manageComponent: props.manageComponentView, manageCompomentItem: { currentComponentName: ComponentNameEnum.AddToolboxTalk, isAddClient: true, breadCrumItems: breadCrumItems, siteName: props.componentProps.siteName, qCState: props.componentProps.qCState, selectedZoneDetails: props.selectedZoneDetails } });
//                                 props.manageComponentView({ currentComponentName: ComponentNameEnum.AddToolboxTalk, qCStateId: props?.qCStateId, dataObj: props.componentProps.dataObj, originalState: props.originalState || props.componentProps.originalState, isAddNewSite: true, breadCrumItems: breadCrumItems, originalSiteMasterId: props.siteMasterId, siteName: props.componentProps.siteName, qCState: props.componentProps.qCState, selectedZoneDetails: props.selectedZoneDetails });
