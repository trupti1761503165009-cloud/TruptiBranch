...
import { faFileExcel, faCheckCircle, faExclamationCircle } from '@fortawesome/free-solid-svg-icons';
...
interface ExcelUploadCategoriesModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
  provider: any;
...
export const ExcelUploadCategoriesModal: React.FC<ExcelUploadCategoriesModalProps> = ({
  isOpen,
  onClose,
  onSuccess,
  provider
  ...
  const handleParseExcel = async () => {
    ...
    try {
      ...
      if (parsed.success) {
        const validated = ExcelHelper.validateCategoryData(parsed.data);
  ...
  const handleUpload = async () => {
    ...
    for (let i = 0; i < parseResult.data.length; i++) {
      ...
      try {
        const categoryData = {
          ...
          CTDModule: row.ctdModule || '',
          eCTDSection: row.ectdSection || '',
          eCTDSubsection: row.ectdSubsection || '',
          eCTDCode: row.ectdCode || ''
        };

        await provider.createItem(categoryData, ListNames.Categories);
        results.success++;
      } catch (error) {
        results.failed++;
    ...
    if (results.success > 0) {
      showToast({ 
        ...
        message: `Uploaded ${results.success} categories. ${results.failed} failed.`
      });
      onSuccess();
    }
  };
  ...
  return (
    <CustomModal
      ...
      subject="Excel Bulk Upload - Categories"
      ...
      message={
        <div>
          <MessageBar messageBarType={MessageBarType.info} style={{ marginBottom: 16 }}>
            <strong>Excel Format:</strong> Category Name, Document Category, Group, Sub Group, Artifact Name, Template Name, Status, Description, CTD Module, eCTD Section, eCTD Subsection, eCTD Code
          </MessageBar>

          <MessageBar messageBarType={MessageBarType.warning} style={{ marginBottom: 16 }}>
            <strong>Tip:</strong> Use DIA_GMP_vs_eCTD_MAPPING.xlsx as reference for proper CTD/eCTD mappings
          </MessageBar>
          ...
          {!uploadResults && (
            <>
              ...
              {isProcessing && uploadProgress > 0 && (
                <div style={{ marginTop: 20 }}>
                  <ProgressIndicator label="Uploading categories..." percentComplete={uploadProgress / 100} />
                </div>
              )}
            </>
          )}
