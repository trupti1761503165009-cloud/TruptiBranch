...
const authFile = 'playwright/.auth/hr.json';

setup('authenticate as HR', async ({ page }) => {
  const username = process.env.PLAYWRIGHT_HR_USERNAME;
  const password = process.env.PLAYWRIGHT_HR_PASSWORD;

  if (!username || !password) {
    throw new Error('Please set PLAYWRIGHT_HR_USERNAME and PLAYWRIGHT_HR_PASSWORD in .env');
  }
