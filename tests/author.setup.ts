...
const authFile = 'playwright/.auth/author.json';

setup('authenticate as Author', async ({ page }) => {
  const username = process.env.PLAYWRIGHT_AUTHOR_USERNAME;
  const password = process.env.PLAYWRIGHT_AUTHOR_PASSWORD;

  if (!username || !password) {
    throw new Error('Please set PLAYWRIGHT_AUTHOR_USERNAME and PLAYWRIGHT_AUTHOR_PASSWORD in .env');
  }
