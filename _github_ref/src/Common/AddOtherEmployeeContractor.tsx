...
export const AddOtherEmployeeContractor: React.FunctionComponent<IAddEmployeeProps> = (props: IAddEmployeeProps): React.ReactElement => {

    const tooltipId = useId('tooltip');
    const [allEmailOptions, setAllEmailOptions] = React.useState<any[]>([]);
    const [employeeOptions, setEmployeeOptions] = React.useState<any[]>([]);
    const [contractorOptions, setContractorOptions] = React.useState<any[]>([]);
    ...
    const [ContractorData, setContractorData] = React.useState<any[]>([]);
    const [addedContracorValues, setAddedContracorValues] = React.useState<string[]>([]);
    ...
    const validateInput = (): boolean => {
        ...
        if (props.selectedAttendeeType === 'Other Contractor') {
            const emailExists = selectedItem
                ? addedContracorValues.filter(email => email !== selectedItem.Email).includes(objData.email)
                : addedContracorValues.includes(objData.email);

            if (emailExists) {
                const matchedRecord = ContractorData.find(record => record.Email === objData.email);
                if (matchedRecord) {
                    const stateId = matchedRecord.State.map((item: any) => item.lookupId)
    ...
    const onClickSave = async (evt: { preventDefault: () => void; }) => {
        try {
            ...
            if (!isValid) {
                ...
                let listName = ListNames.QuaycleanEmployee;
                if (props.selectedAttendeeType === 'Other Contractor') {
                    listName = ListNames.QuaycleanContractor;
                }
                try {
                    if (responseId.current > 0) {
                        let updatedEmployee = await props.provider.updateItemWithPnP(statedata, listName, responseId.current);
                        ...
                        if (!selectedItem) {
                            const newEmployee = await props.provider.createItem(data, listName);
                            setNewEmployeeList(prevList => [
                                ...prevList,
                                {
                            ...
                            if (props.selectedAttendeeType === 'Other Contractor') {
                                drpOption.push({ value: `con_${newEmployee.data.Id}`, key: newEmployee.data.Id, text: newEmployee.data.Title, label: `${newEmployee.data.Title} (Contractor)`, type: 'contractor', uniqueValue: Number(newEmployee.data.Id) });
                            } else {
                                drpOption.push({ value: `emp_${newEmployee.data.Id}`, key: newEmployee.data.Id, text: newEmployee.data.Title, label: `${newEmployee.data.Title}`, type: 'employee', uniqueValue: Number(newEmployee.data.Id) });
                            }

                        } else {
                            let updatedEmployee = await props.provider.updateItemWithPnP(data, listName, selectedItem.ID);
                            updatedEmployee = {
                                ...updatedEmployee,
                                ID: selectedItem.ID,
                            };
    ...
    const getQuaycleanEmployeeList = async (): Promise<void> => {
        ...
        employeeList.map((Employee: any) => {
            ...
            dropvalue.push({ value: `emp_${Employee.ID}`, key: Number(Employee.ID), text: FullName, label: FullName, type: 'employee', uniqueValue: Number(Employee.ID) });
        });
    ...
    const getQuaycleanContractorList = async (): Promise<void> => {
        let dropvalue: any = [];
        const label = `-- All --`;
        if (props.AllOption === true) {
            dropvalue.push({ key: '', text: '', value: 'All', label: label });
        ...
        setAddedContracorValues(emailArray);
        let contractorList = results.filter((data: any) =>
            data.State.some((state: any) => state.lookupId === props.StateId)
        );

        contractorList.map((Employee: any) => {
            let FullName = Employee.FirstName + " " + Employee.LastName;
            dropvalue.push({ value: `con_${Employee.ID}`, key: Number(Employee.ID), text: `${FullName}`, label: `${FullName} (Contractor)`, type: 'contractor', uniqueValue: Number(Employee.ID) });
        });
        setContractorOptions(dropvalue);
        // }).catch((error) => {
        //     console.log(error);
        // });
    };
    ...
    React.useEffect(() => {
        ...
        // } else {
        //     setContractorData([]);
        //     setAddedContracorValues([]);
        //     setContractorOptions([]);
        // }
        setNewEmployeeList([]);
    }, [props.selectedAttendeeType]);

    React.useEffect(() => {
        const allOptions = [...employeeOptions, ...contractorOptions];
        setAllEmailOptions(allOptions);
    }, [employeeOptions, contractorOptions]);
    ...
    return <>
        ...
        <div>
            {props.selectedAttendeeType != 'Quayclean Employee' && <div className="ttadd-10 mt-2">
