...
import { NO_SITE_CATEGORY_ID } from '../Constants/CommonConstants';
...
export const MultipleSiteFilterWithCategory: React.FunctionComponent<ISiteFilterProps> = (props: ISiteFilterProps): React.ReactElement => {
    ...
    const getSitenameList = (): void => {
        ...
        if (selectedStates.length > 0) {
            filters.push(selectedStates.map((id: any) => `QCStateId eq ${id}`).join(" or "));
        }

        if (props.siteCategoryId === NO_SITE_CATEGORY_ID) {
            filters.push(`SiteCategoryId eq null`);
        } else if (props.siteCategoryId) {
            filters.push(`SiteCategoryId eq ${props.siteCategoryId}`);
        }
        ...
        props.provider.getItemsByQuery(queryStringOptions).then((response: any) => {
            if (props.isPermissionFiter) {
                if (props.loginUserRoleDetails) {
                    const { isAdmin, isStateManager, isSiteManager, isUser, stateManagerStateItem, siteManagerItem, Id } = props.loginUserRoleDetails;
                    if (isAdmin) {
    ...
    React.useEffect(() => {
        getSitenameList();
    }, [props.selectedState, props.siteCategoryId]);
