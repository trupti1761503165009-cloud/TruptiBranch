import { DatePicker, DayOfWeek, IDropdownOption, PrimaryButton } from "@fluentui/react";
...
export const DateRangeFilterDefault: React.FunctionComponent<IDaterangeFilterProps> = (props: IDaterangeFilterProps): React.ReactElement => {
    ...
    const optionDate: any[] = [
        ...
        { value: 'All', key: 'All', text: 'All', label: 'All ' }
    ];

    const [firstDayOfWeek,] = React.useState(DayOfWeek.Sunday);
    const [selectedOption, setSelectedOption] = React.useState<IDropdownOption>(!!props?.selectedOption ? props?.selectedOption : { key: "", text: "" });
    ...
    const _onChangeRangeOption = (option: any, actionMeta: ActionMeta<any>): void => {
        ...
        if (!!option && option?.text != "Custom Range") {
    ...
    return <>
        ...
        <div className={!props.isHideWidth ? "ms-Grid-col ms-sm12 ms-md2 ms-lg2" : ""} >
