...
export enum ValidationType {
    ...
    Excel = "excel",
    PDF = "pdf",
    NumberAndFloat = "Number",
    MultiEmail = "MultiEmail",
    NumberLength13 = "NumberLength13",
    CommaNumbers = "CommaNumbers"
}

const validEmail = new RegExp('^[a-zA-Z0-9._:$!%-]+@[a-zA-Z0-9.-]+.[a-zA-Z]$');
const validNumber = new RegExp(/^-?\d+(\.\d+)?$/);
const checkNumberandfloat = /^-?\d+(\.\d+)?$/;
const checkNumberWithComma = /^-?(?:\d{1,3}(?:,\d{3})+|\d+)(\.\d+)?$/;
        ...
        for (let i = 0; i < field.type.length; i++) {
            ...
            switch (element) {
                ...
                case ValidationType.PDF:
                    ...
                    break
                case ValidationType.NumberAndFloat:
                    if (fieldsObj["isValid" + field.fieldName] === undefined || fieldsObj["isValid" + field.fieldName]) {
                        fieldsObj["isValid" + field.fieldName] = checkNumberandfloat.test(data[field.fieldName]);
                        if (!fieldsObj["isValid" + field.fieldName]) {
                            fieldsObj["errorMessage" + field.fieldName] = field.displayText + " Contains Only Digit.";
                            isValid = false;
                        }
                    }
                    break;
                case ValidationType.CommaNumbers:
                    if (fieldsObj["isValid" + field.fieldName] === undefined || fieldsObj["isValid" + field.fieldName]) {
                        fieldsObj["isValid" + field.fieldName] = checkNumberWithComma.test(data[field.fieldName]);
                        if (!fieldsObj["isValid" + field.fieldName]) {
                            fieldsObj["errorMessage" + field.fieldName] = field.displayText + " Contains Only Digit.";
                            isValid = false;
                        }
                    ...
                    break;
                case ValidationType.MultiEmail:
                    if (fieldsObj["isValid" + field.fieldName] === undefined || fieldsObj["isValid" + field.fieldName]) {
                        // fieldsObj["isValid" + field.fieldName] = validEmail.test(data[field.fieldName]);
                        fieldsObj["isValid" + field.fieldName] = data[field.fieldName]?.split(';').every((email: string) => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.trim()));
                        if (!fieldsObj["isValid" + field.fieldName]) {
                            fieldsObj["errorMessage" + field.fieldName] = field.displayText + " is not a valid email address.";
                            isValid = false;
                        }
                    ...
                    break;
                case ValidationType.NumberLength13:
                    if (fieldsObj["isValid" + field.fieldName] === undefined || fieldsObj["isValid" + field.fieldName]) {
                        const value = data[field.fieldName].replace(/_/g, '');
                        if (value.length != 13) {
                            fieldsObj["errorMessage" + field.fieldName] = field.displayText + " is not valid.";
                            isValid = false;
                            fieldsObj["isValid" + field.fieldName] = false;
                        }
                    }
