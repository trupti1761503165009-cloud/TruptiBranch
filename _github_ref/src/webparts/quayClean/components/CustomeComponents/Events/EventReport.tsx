...
const EventReport: React.FC<Props> = ({ data, isChartOnly }) => {
    ...
    const filteredData = data.filter(item => item.EntityType === UserActionEntityTypeEnum.Event);
    ...
    const drawLevel2 = () => {
        ...
        chartInstance.current?.off('click');
        chartInstance.current?.setOption({
            title: { text: `Event Entities`, left: 'center' },
    ...
    const drawLevel3 = (entityName: string) => {
        ...
        chartInstance.current?.off('click');
        chartInstance.current?.setOption({
            title: { text: `Event Actions on ${entityName}`, left: 'center' },
    ...
    const drawLevel4 = (entityName: string, actionType: string) => {
        ...
        chartInstance.current?.off('click');
        chartInstance.current?.setOption({
            title: { text: `Event by Users for ${actionType} on ${entityName}`, left: 'center' },
                ...
                <div>
                    <div className="ims-Site-chart-card mt-3">
                        <div className="chart-header d-flex justify-content-between align-items-center dflex">
                            <div>
                                <Label className="chart-label">Event Activity Log Report</Label>
                            </div>
                            {level > 1 && (
                                <PrimaryButton
                                    text="Back"
                    ...
                    {((isChartOnly == undefined || isChartOnly == false) && (!!filteredData && filteredData.length > 0)) && (
                        <EventReportList filteredData={filteredData} />
                    )}
                </div>
                : <div>
