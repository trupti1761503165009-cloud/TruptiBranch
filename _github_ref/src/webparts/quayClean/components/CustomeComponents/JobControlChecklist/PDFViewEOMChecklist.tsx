...
export const PDFViewEOMChecklist = (props: IHelpDeskFormProps) => {
    ...
    const onClickClose = () => {
        ...
        if (isSiteLevelComponent) {
            ...
            const breadCrumItems = removeElementOfBreadCrum(props.breadCrumItems);
            manageComponentView({ currentComponentName: ComponentNameEnum.ViewEOMChecklist, breadCrumItems: breadCrumItems });
        }
    };
    ...
    const onClickDownload = async (): Promise<void> => {
        let fileblob: any = await generateAndSaveKendoPDF("pdfJCC", SiteName + '- Manager’s Monthly KPI’s', false, true);
    };
    ...
    const onClickSendEmail = async (): Promise<void> => {
        ...
        if (!isTitleEmpty && !isEmailEmpty && !isEmailInvalid) {
            const fileName = `${SiteName} Manager’s Monthly KPI’s`;
            ...
            props.provider.createItem(insertData, ListNames.SendEmailTempList).then((item: any) => {
                props.provider.uploadAttachmentToList(ListNames.SendEmailTempList, file, item.data.Id).then(async () => {
                    ...
                    const logObj = {
                        ...
                        Details: `Send Email Manager’s Monthly KPI’s to ${sendToEmail}`,
                            ...
                            <div>
                                <div className="asset-card-2-header-jcc">
                                    <div className="card-jcc-header">
                                        ...
                                        <h2 className="card-header-head">Manager’s Monthly KPI’s</h2>
                                    </div>
