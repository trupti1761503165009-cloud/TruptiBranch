...
export const AssociateEOMChecklist = (props: IHelpDeskFormProps) => {
    ...
    const onClickClose = () => {
        ...
        if (isSiteLevelComponent) {
            ...
            const breadCrumItems = removeElementOfBreadCrum(props.breadCrumItems);
            props.manageComponentView({
                currentComponentName: ComponentNameEnum.AddNewSite, dataObj: props.componentProps.dataObj, breadCrumItems: breadCrumItems, siteMasterId: props.originalSiteMasterId, isShowDetailOnly: true, siteName: props.componentProps.siteName, qCState: props.componentProps.originalState, pivotName: "ViewJobControlChecklistKey"
            });
        }
    };
    const onClickSaveOrUpdate = async () => {
        ...
        if (!!props?.siteMasterId && props?.siteMasterId > 0) {
            ...
            try {
                ...
                await Promise.all(
                    UpdateBatchData.current.map(async (item: any) => {
                        ...
                        await props.provider.updateItemWithPnP(updateObj, ListNames.EOMChecklistDetails, Number(updateId));
                    })
                );
                ...
                if (UpdateIdMaster > 0 && isCompletedUpdate) {
                    await props.provider.updateItemWithPnP(updateCompletedObj, ListNames.EOMChecklistMaster, UpdateIdMaster);
                }

                await props.provider.createItemInBatch(UpdateHistoryData.current, ListNames.EOMChecklistHistory);
            ...
            try {
                await Promise.all([
                    props.provider.createItemInBatch(BatchData.current, ListNames.EOMChecklistDetails),
                    props.provider.createItem(MasterData.current, ListNames.EOMChecklistMaster),
                    props.provider.createItemInBatch(HistoryData.current, ListNames.EOMChecklistHistory),
                ]);
    ...
    const getOptionList = (): void => {
        let dropvalue: any = [];
        props.provider.choiceOption(ListNames.EOMChecklistDetails, "Status").then((response) => {
    ...
    const getOptionList2 = (): void => {
        let dropvalue: any = [];
        props.provider.choiceOption(ListNames.EOMChecklist, "Frequency").then((response) => {
            response.map((value: any) => {
                dropvalue.push({ value: value, key: value, text: value, label: value });
            });
                            ...
                            <div className="asset-card-2-header">
                                <div className="card-jcc-header dflex justify-content-between align-items-center">
                                    <div className="dflex align-items-center">
                                        ...
                                        <h2 className="card-header-head" style={{ marginLeft: '10px' }}>Manager’s Monthly KPI’s</h2>
                                    </div>
                                    <PrimaryButton
