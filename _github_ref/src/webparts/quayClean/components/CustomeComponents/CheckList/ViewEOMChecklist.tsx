...
import { DefaultButton, DialogFooter, DialogType, FocusTrapZone, GroupHeader, IColumn, IDetailsGroupRenderProps, IGroupHeaderProps, Layer, Link, mergeStyleSets, Overlay, Popup, PrimaryButton, SelectionMode, TooltipHost } from "@fluentui/react";
...
import { EOMChecklistHistory } from "./EOMChecklistHistory";
export interface IEOMChecklistListProps {
...
export interface IEOMChecklistListState {
}

export const ViewEOMChecklist = (props: IEOMChecklistListProps) => {
    ...
    const _QuestionMasterData = () => {
        ...
        try {
            ...
            const queryStringOptions: IPnPQueryOptions = {
                select: select,
                filter: filter,
                listName: ListNames.EOMChecklistMaster,
            };
    ...
    const _QuestionData = () => {
        ...
        try {
            ...
            const queryStringOptions: IPnPQueryOptions = {
                ...
                expand: expand,
                filter: filter,
                listName: ListNames.EOMChecklistDetails,
            };
            provider.getItemsByQuery(queryStringOptions).then((results: any[]) => {
                if (!!results) {
                    ...
                    // const groupedData = filteredData.reduce((acc: any, item: any, index: any) => {
                    //     const key = item.MonthYear;
                    //     if (!acc[key]) {
                    //         acc[key] = { key: `group${key}${index}`, name: `${key}`, startIndex: index, count: 0, level: 0 };
                    //     }
    ...
    const _userActivityLog = async () => {
        ...
        try {
            ...
            const queryStringOptions: IPnPQueryOptions = {
                ...
                filter: `Email eq '${currentUserRoleDetail?.emailId}' and EntityId eq '${props?.siteMasterId}' and SiteNameId eq '${orgSiteId}' and EntityType eq '${UserActionEntityTypeEnum.EOMChecklist}' and ActionType eq 'Details View' and Created ge datetime'${todayDate}T00:00:00Z' and Created le datetime'${todayDate}T23:59:59Z'`
            };
            ...
            if (results && results.length > 0) {
                ...
                const logObj = {
                    ...
                    EntityType: UserActionEntityTypeEnum.EOMChecklist,
    ...
    const onClickYes = () => {
        ...
        breadCrumItems.push({ text: "Update", key: "Update", currentCompomnetName: ComponentNameEnum.AddNewSite, onClick: onBreadcrumbItemClicked, manageComponent: props.manageComponentView, manageCompomentItem: { currentComponentName: ComponentNameEnum.AssociateEOMChecklist, siteMasterId: 1, isShowDetailOnly: false, breadCrumItems: breadCrumItems } });
        props.manageComponentView({
            currentComponentName: ComponentNameEnum.AssociateEOMChecklist, Month: CurrentMonth, Year: CurrentYear, dataObj: props.dataObj, siteMasterId: 1, isShowDetailOnly: false, breadCrumItems: breadCrumItems, originalSiteMasterId: props.siteMasterId
        });
    };
    ...
    const onClickHeaderView = (item: any) => {
        ...
        if (props?.originalSiteMasterId) {
            let breadCrumItems: IBreadCrum[] = props.breadCrumItems;
            breadCrumItems.push({ text: "PDF View", key: 'PDF View', currentCompomnetName: ComponentNameEnum.PDFViewEOMChecklist, onClick: onBreadcrumbItemClicked, manageComponent: props.manageComponentView, manageCompomentItem: { currentComponentName: ComponentNameEnum.PDFViewEOMChecklist, isAddNewSite: true, breadCrumItems: breadCrumItems, dataObj: props.dataObj, originalState: props.originalState } });
            props.manageComponentView({ currentComponentName: ComponentNameEnum.PDFViewEOMChecklist, originalState: props.originalState, dataObj: props.dataObj, isAddNewSite: true, breadCrumItems: breadCrumItems, originalSiteMasterId: props.siteMasterId });
        } else {
            let breadCrumItems: IBreadCrum[] = props.breadCrumItems;
            breadCrumItems.push({ text: "PDF View", key: 'PDF View', currentCompomnetName: ComponentNameEnum.PDFViewEOMChecklist, onClick: onBreadcrumbItemClicked, manageComponent: props.manageComponentView, manageCompomentItem: { currentComponentName: ComponentNameEnum.PDFViewEOMChecklist, isAddNewSite: true, breadCrumItems: breadCrumItems, dataObj: props.dataObj, originalState: props.originalState } });
            props.manageComponentView({ currentComponentName: ComponentNameEnum.PDFViewEOMChecklist, originalState: props.originalState, dataObj: props.dataObj, isAddNewSite: true, breadCrumItems: breadCrumItems, originalSiteMasterId: selectedSiteName });
        }
        setIsLoading(false);
    };
    ...
    const onClickHeaderEdit = (item: any) => {
        ...
        if (props?.originalSiteMasterId) {
            if (Monthss === Month && Yearss === Year) {
                setisDisplayEDbtn(false);
                let breadCrumItems: IBreadCrum[] = props.breadCrumItems;
                breadCrumItems.push({ text: "Update", key: "Update", currentCompomnetName: ComponentNameEnum.AddNewSite, onClick: onBreadcrumbItemClicked, manageComponent: props.manageComponentView, manageCompomentItem: { currentComponentName: ComponentNameEnum.AssociateEOMChecklist, originalState: props.originalState, siteMasterId: 1, isShowDetailOnly: false, breadCrumItems: breadCrumItems } });
                props.manageComponentView({
                    currentComponentName: ComponentNameEnum.AssociateEOMChecklist, Month: Monthss, Year: Yearss, originalState: props.originalState, dataObj: props.dataObj, siteMasterId: 1, isShowDetailOnly: false, breadCrumItems: breadCrumItems, originalSiteMasterId: props.siteMasterId
                });
            } else {
                showPopup();
            }
            ...
            if (Monthss === Month && Yearss === Year) {
                ...
                breadCrumItems.push({ text: "Update", key: "Update", currentCompomnetName: ComponentNameEnum.AddNewSite, onClick: onBreadcrumbItemClicked, manageComponent: props.manageComponentView, manageCompomentItem: { currentComponentName: ComponentNameEnum.AssociateEOMChecklist, originalState: props.originalState, siteMasterId: 1, isShowDetailOnly: false, breadCrumItems: breadCrumItems } });
                props.manageComponentView({
                    currentComponentName: ComponentNameEnum.AssociateEOMChecklist, Month: Monthss, Year: Yearss, originalState: props.originalState, dataObj: props.dataObj, siteMasterId: 1, isShowDetailOnly: false, breadCrumItems: breadCrumItems, originalSiteMasterId: selectedSiteName
                });
            } else {
                showPopup();
            }
    ...
    if (hasError) {
        ...
        return <>
            {HistoryView && Month !== "" && Year !== "" && <EOMChecklistHistory
            ...
            {!!props.siteMasterId ?
                <div className={!!props.siteMasterId ? "" : "boxCard"}>
                    {!props.siteMasterId && <div className="ms-Grid-row">
                        <div className="ms-Grid-col ms-sm12 ms-md12 ms-lg12 ">
                            <h1 className="mainTitle">Manager’s Monthly KPI’s</h1>
                        </div>
                    </div>}
                    <div className="ms-Grid mt-5">
                            ...
                            <>
                                <div className={window.innerWidth > 768 ? "dflex" : "jcc-refresh"}>
                                    ...
                                    {isVisibleCrud.current && recordFound === false && <PrimaryButton text="Add" className="btn btn-primary "
                                        onClick={() => {
                                            let breadCrumItems: IBreadCrum[] = props.breadCrumItems;
                                            breadCrumItems.push({ text: "Add Form", key: 'Add Form', currentCompomnetName: ComponentNameEnum.AssociateEOMChecklist, onClick: onBreadcrumbItemClicked, manageComponent: props.manageComponentView, manageCompomentItem: { currentComponentName: ComponentNameEnum.AssociateEOMChecklist, originalState: props.originalState, isAddNewSite: true, breadCrumItems: breadCrumItems, dataObj: props.dataObj } });
                                            props.manageComponentView({ currentComponentName: ComponentNameEnum.AssociateEOMChecklist, originalState: props.originalState, dataObj: props.dataObj, isAddNewSite: true, breadCrumItems: breadCrumItems, originalSiteMasterId: props.siteMasterId });
                                            setIsLoading(false);
                                        }}
                                    />}
                                </div>
                ...
                <div className={!!props.siteMasterId ? "" : "boxCard"}>
                    <div className="ms-Grid-row">
                        ...
                        <div className={window.innerWidth > 768 ? "ms-Grid-col ms-sm10 ms-md10 ms-lg10 ms-xl10" : "ms-Grid-col ms-sm12 ms-md12 ms-lg12 ms-xl12"} style={{ borderLeft: '1px solid #e5e5e5' }}>
                            {!props.siteMasterId && <div className="ms-Grid-row">
                                <div className="ms-Grid-col ms-sm12 ms-md12 ms-lg12 ">
