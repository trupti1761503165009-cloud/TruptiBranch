...
import LinkDocumentReportList from './LinkDocumentReportList';
...
const StateLinkDocumentReport: React.FC<Props> = ({ data, isChartOnly }) => {
    ...
    const filteredData = data.filter(item => item.EntityType === UserActionEntityTypeEnum.LinkDocument || item.EntityType === UserActionEntityTypeEnum.Document);
    ...
    const drawLevel0 = () => {
        ...
        chartInstance.current?.setOption({
            title: { text: 'Link Document Activity by State', left: 'center' },
    ...
    const drawLevel1 = (stateName: string) => {
        ...
        chartInstance.current?.off('click');
        chartInstance.current?.setOption({
            title: { text: 'Link Document Activity by Site Name', left: 'center' },
    ...
    const drawLevel2 = (siteName: string) => {
        ...
        chartInstance.current?.off('click');
        chartInstance.current?.setOption({
            title: { text: `Link Document Entities in ${siteName}`, left: 'center' },
    ...
    const drawLevel3 = (siteName: string, entityName: string) => {
        ...
        chartInstance.current?.off('click');
        chartInstance.current?.setOption({
            title: { text: `Link Document Actions on ${entityName} in ${siteName}`, left: 'center' },
            ...
            ${items.map(a => `User: ${a.UserName} @ ${a.Modified}`).join('<br/>')}
          `;
                }
            },
            series: [{
    ...
    const drawLevel4 = (siteName: string, entityName: string, actionType: string) => {
        ...
        chartInstance.current?.off('click');
        chartInstance.current?.setOption({
            title: { text: `Link Document by Users for ${actionType} on ${entityName}`, left: 'center' },
            ...
            tooltip: {
                ...
                formatter: function (params: any) {
                    ...
                    return `<strong>${user}</strong><br/>${items.map(a => a.Modified).join('<br/>')}`;
                }
            },
            series: [{
                type: 'bar',
                ...
                <div>
                    <div className="ims-Site-chart-card mt-3">
                        <div className="chart-header d-flex justify-content-between align-items-center dflex">
                            <div>
                                <Label className="chart-label">Link Document Activity Log Report</Label>
                            </div>
                            {level > 0 && (
                                <PrimaryButton
                                    text="Back"
                    ...
                    {((isChartOnly == undefined || isChartOnly == false) && (!!filteredData && filteredData.length > 0)) && (
                        <LinkDocumentReportList filteredData={filteredData} />
                    )}
                </div>
                : <div>
...
export default StateLinkDocumentReport;
