...
import LinkURLReportList from './LinkURLReportList';
...
const StateLinkURLReport: React.FC<Props> = ({ data, isChartOnly }) => {
    ...
    const filteredData = data.filter(item => item.EntityType === UserActionEntityTypeEnum.LinkURL);
    ...
    const drawLevel0 = () => {
        ...
        chartInstance.current?.setOption({
            title: { text: 'Link URL Activity by State', left: 'center' },
            xAxis: { type: 'category', data: states, axisLabel: { rotate: 30 } },
            yAxis: { type: 'value' },
            tooltip: {
                trigger: 'axis',
    ...
    const drawLevel1 = (stateName: string) => {
        ...
        chartInstance.current?.off('click');
        chartInstance.current?.setOption({
            title: { text: 'Link URL Activity by Site Name', left: 'center' },
            xAxis: { type: 'category', data: siteNames, axisLabel: { rotate: 0 } },
            yAxis: { type: 'value' },
            tooltip: {
                trigger: 'axis',
                formatter: function (params: any) {
    ...
    const drawLevel2 = (siteName: string) => {
        ...
        chartInstance.current?.off('click');
        chartInstance.current?.setOption({
            title: { text: `Link URL Entities in ${siteName}`, left: 'center' },
    ...
    const drawLevel3 = (siteName: string, entityName: string) => {
        ...
        chartInstance.current?.off('click');
        chartInstance.current?.setOption({
            title: { text: `Link URL Actions on ${entityName} in ${siteName}`, left: 'center' },
    ...
    const drawLevel4 = (siteName: string, entityName: string, actionType: string) => {
        ...
        chartInstance.current?.off('click');
        chartInstance.current?.setOption({
            title: { text: `Link URL by Users for ${actionType} on ${entityName}`, left: 'center' },
                ...
                <div>
                    <div className="ims-Site-chart-card mt-3">
                        <div className="chart-header d-flex justify-content-between align-items-center dflex">
                            <div>
                                <Label className="chart-label">Link URL Activity Log Report</Label>
                            </div>
                            {level > 0 && (
                                <PrimaryButton
                                    text="Back"
                    ...
                    {/* {((isChartOnly == undefined || isChartOnly == false) && (!!filteredData && filteredData.length > 0)) && (
                        <LinkURLReportList filteredData={filteredData} />
                    )} */}
                </div>
                : <div>
                    <div className="ims-Site-chart-card mt-3">
...
export default StateLinkURLReport;
