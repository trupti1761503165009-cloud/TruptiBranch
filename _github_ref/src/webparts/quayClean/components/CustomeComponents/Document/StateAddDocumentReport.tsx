...
import AddDocumentReportList from './AddDocumentReportList';
...
const StateAddDocumentReport: React.FC<Props> = ({ data, isChartOnly }) => {
    ...
    const filteredData = data.filter(item => item.EntityType === UserActionEntityTypeEnum.AddDocument);
    ...
    const drawLevel0 = () => {
        ...
        chartInstance.current?.setOption({
            title: { text: 'Add Document Activity by State', left: 'center' },
    ...
    const drawLevel1 = (stateName: string) => {
        ...
        chartInstance.current?.off('click');
        chartInstance.current?.setOption({
            title: { text: 'Add Document Activity by Site Name', left: 'center' },
    ...
    const drawLevel2 = (siteName: string) => {
        ...
        chartInstance.current?.off('click');
        chartInstance.current?.setOption({
            title: { text: `Add Document Entities in ${siteName}`, left: 'center' },
    ...
    const drawLevel3 = (siteName: string, entityName: string) => {
        ...
        chartInstance.current?.off('click');
        chartInstance.current?.setOption({
            title: { text: `Add Document Actions on ${entityName} in ${siteName}`, left: 'center' },
    ...
    const drawLevel4 = (siteName: string, entityName: string, actionType: string) => {
        ...
        chartInstance.current?.off('click');
        chartInstance.current?.setOption({
            title: { text: `Add Document by Users for ${actionType} on ${entityName}`, left: 'center' },
            ...
            {!!filteredData && filteredData.length > 0 ? <div>
                <div className="ims-Site-chart-card mt-3">
                    <div className="chart-header d-flex justify-content-between align-items-center dflex">
                        <div>
                            <Label className="chart-label">Document Activity Log Report</Label>
                        </div>
                        {level > 0 && (
                            <PrimaryButton
                                text="Back"
                ...
                {/* {((isChartOnly == undefined || isChartOnly == false) && (!!filteredData && filteredData.length > 0)) && (
                    <AddDocumentReportList filteredData={filteredData} />
                )} */}
            </div>
                : <div>
...
export default StateAddDocumentReport;
