...
import JobControlChecklistReportList from './JobControlChecklistReportList';
...
const JobControlChecklistReport: React.FC<Props> = ({ data, isChartOnly }) => {
    ...
    const filteredData = data.filter(item => item.EntityType === UserActionEntityTypeEnum.JobControlChecklist);
    ...
    const drawLevel1 = () => {
        ...
        chartInstance.current?.setOption({
            title: {
                text: `Monthly KPI's Activity by Site Name`, left: 'center'
            },
    ...
    const drawLevel2 = (siteName: string) => {
        ...
        chartInstance.current?.off('click');
        chartInstance.current?.setOption({
            title: { text: `Monthly KPI's Entities in ${siteName}`, left: 'center' },
    ...
    const drawLevel3 = (siteName: string, entityName: string) => {
        ...
        chartInstance.current?.off('click');
        chartInstance.current?.setOption({
            title: { text: `Monthly KPI's Actions on ${entityName} in ${siteName}`, left: 'center' },
    ...
    const drawLevel4 = (siteName: string, entityName: string, actionType: string) => {
        ...
        chartInstance.current?.off('click');
        chartInstance.current?.setOption({
            title: { text: `Monthly KPI's by Users for ${actionType} on ${entityName}`, left: 'center' },
                ...
                <div>
                    <div className="ims-Site-chart-card mt-3">
                        <div className="chart-header d-flex justify-content-between align-items-center dflex">
                            <div>
                                <Label className="chart-label">Monthly KPI's Activity Log Report</Label>
                            </div>
                            {level > 1 && (
                                <PrimaryButton
                                    text="Back"
                    ...
                    {(((isChartOnly == undefined || isChartOnly == false)) && (!!filteredData && filteredData.length > 0)) && (
                        <JobControlChecklistReportList filteredData={filteredData} />
                    )}
                </div>
                : <div>
                    <div className="ims-Site-chart-card mt-3">
...
export default JobControlChecklistReport;
