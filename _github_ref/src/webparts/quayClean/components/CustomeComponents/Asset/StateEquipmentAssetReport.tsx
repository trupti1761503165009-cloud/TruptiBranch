...
import EquipmentAssetReportList from './EquipmentAssetReportList';
...
const StateEquipmentAssetReport: React.FC<Props> = ({ data, isChartOnly }) => {
    ...
    const filteredData = data.filter(item => item.EntityType === UserActionEntityTypeEnum.EquipmentAsset);
    ...
    const drawLevel0 = () => {
        ...
        chartInstance.current?.setOption({
            title: { text: 'Equipment/Asset Activity by State', left: 'center' },
    ...
    const drawLevel1 = (stateName: string) => {
        ...
        chartInstance.current?.off('click');
        chartInstance.current?.setOption({
            title: { text: `Sites in ${stateName}`, left: 'center' },
    ...
    const drawLevel2 = (siteName: string) => {
        ...
        chartInstance.current?.off('click');
        chartInstance.current?.setOption({
            title: { text: `Equipment/Asset by Entities in ${siteName}`, left: 'center' },
    ...
    const drawLevel3 = (siteName: string, entityName: string) => {
        ...
        chartInstance.current?.off('click');
        chartInstance.current?.setOption({
            title: { text: `Equipment/Asset Actions on ${entityName} in ${siteName}`, left: 'center' },
    ...
    const drawLevel4 = (siteName: string, entityName: string, actionType: string) => {
        ...
        chartInstance.current?.off('click');
        chartInstance.current?.setOption({
            title: { text: `Equipment/Asset by Users for ${actionType} on ${entityName}`, left: 'center' },
                ...
                <div>
                    <div className="ims-Site-chart-card mt-3">
                        <div className="chart-header d-flex justify-content-between align-items-center dflex">
                            <div>
                                <Label className="chart-label">Equipment/Asset Activity Log Report</Label>
                            </div>
                            {level > 0 && (
                                <PrimaryButton
                                    text="Back"
...
export default StateEquipmentAssetReport;
