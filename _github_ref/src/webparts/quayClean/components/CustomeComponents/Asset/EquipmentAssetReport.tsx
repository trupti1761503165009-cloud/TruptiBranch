...
const EquipmentAssetReport: React.FC<Props> = ({ data, isChartOnly }) => {
  ...
  const drawLevel1 = () => {
    ...
    chartInstance.current?.setOption({
      title: { text: 'Equipment/Asset Activity by Site Name', left: 'center' },
  ...
  const drawLevel2 = (siteName: string) => {
    ...
    chartInstance.current?.off('click');
    chartInstance.current?.setOption({
      title: { text: `Equipment/Asset Entities in ${siteName}`, left: 'center' },
      ...
      {!!filteredData && filteredData.length > 0 ?
        <div>
          ...
          {((isChartOnly == undefined || isChartOnly == false) && (!!filteredData && filteredData.length > 0)) && (
            <EquipmentAssetReportList filteredData={filteredData} />
          )}
        </div>
        : <div>
...
export default EquipmentAssetReport;
