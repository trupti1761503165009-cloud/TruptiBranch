...
const StatePeriodicReport: React.FC<Props> = ({ data, isChartOnly }) => {
    ...
    const filteredData = data.filter(item => item.EntityType === UserActionEntityTypeEnum.Periodic);
    ...
    const drawLevel0 = () => {
        ...
        chartInstance.current?.setOption({
            title: { text: 'Periodic Activity by State', left: 'center' },
    ...
    const drawLevel1 = (stateName: string) => {
        ...
        chartInstance.current?.off('click');
        chartInstance.current?.setOption({
            title: { text: 'Periodic Activity by Site Name', left: 'center' },
    ...
    const drawLevel2 = (siteName: string) => {
        ...
        chartInstance.current?.off('click');
        chartInstance.current?.setOption({
            title: { text: `Periodic Entities in ${siteName}`, left: 'center' },
    ...
    const drawLevel3 = (siteName: string, entityName: string) => {
        ...
        chartInstance.current?.off('click');
        chartInstance.current?.setOption({
            title: { text: `Periodic Actions on ${entityName} in ${siteName}`, left: 'center' },
    ...
    const drawLevel4 = (siteName: string, entityName: string, actionType: string) => {
        ...
        chartInstance.current?.off('click');
        chartInstance.current?.setOption({
            title: { text: `Periodic by Users for ${actionType} on ${entityName}`, left: 'center' },
                ...
                <div>
                    <div className="ims-Site-chart-card mt-3">
                        <div className="chart-header d-flex justify-content-between align-items-center dflex">
                            <div>
                                <Label className="chart-label">Periodic Activity Log Report</Label>
                            </div>
                            {level > 0 && (
                                <PrimaryButton
                                    text="Back"
                    ...
                    {/* {((isChartOnly == undefined || isChartOnly == false) && (!!filteredData && filteredData.length > 0)) && (
                        <PeriodicReportList filteredData={filteredData} />
                    )} */}
                </div>
                : <div>
                    <div className="ims-Site-chart-card mt-3">
...
export default StatePeriodicReport;
