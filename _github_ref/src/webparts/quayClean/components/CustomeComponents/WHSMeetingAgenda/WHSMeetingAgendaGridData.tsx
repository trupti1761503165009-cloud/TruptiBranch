...
import { IWHSCommitteeMeetingMaster } from "../WHSForms/IAddWHSMeetingFroms";
import { IWHSMeetingAgendaGridProps } from "./WHSMeetingAgendaGrid";
...
export const WHSMeetingAgendaGridData = (props: IWHSMeetingAgendaGridProps) => {
    ...
    //     setTimeout(() => {
    //         if (signatureItems && !!state.items && state.items.length > 0 && isSetSignatureData) {
    ...
    const getItemsWHSMaster = async () => {
        ...
        const baseViewFields = [
            "MinutesCirculatedTo", "MeetingDate", "Apologies", "Attendees", "Other",
            "EndTime", "StartTime", "Location", "ID", "Title", "Created", "Editor",
            "Modified", "Author", "SignatureDoneBy"
        ];
        ...
        if (selectedItem?.key === "Top 30 Records") {
            camlQuery = new CamlBuilder()
                ...
                .Query()
                .Where()
                .ChoiceField("WHSCommitteeMeetingType").EqualTo(WHSCommitteeMeetingTypeEnum.WHSCommitteeMeetingAgenda)
                .ToString();
            ...
            const startDate = filterFromDate || moment().subtract(29, "days").format("YYYY-MM-DD");
            const endDate = filterToDate || moment().format("YYYY-MM-DD");
            camlQuery = new CamlBuilder()
                .View(baseViewFields)
                .Scope(CamlBuilder.ViewScope.RecursiveAll)
                .RowLimit(5000, true)
    ...
    }, [state.isReload, state.selectedLocation, filterFromDate, filterToDate])
