...
const SubCategoryCountsByUser: React.FC<ReportProps> = ({ data, width, height, title }) => {
    ...
    const renderChart = () => {
        ...
        const users = unique(data.map(d => d.ReportedBy ?? "Unknown"));

        const userCounts = users
            .map(user => ({
                user,
                total: data.filter(d => (d.ReportedBy ?? "Unknown") === user).length
            }))
...
export default SubCategoryCountsByUser;
