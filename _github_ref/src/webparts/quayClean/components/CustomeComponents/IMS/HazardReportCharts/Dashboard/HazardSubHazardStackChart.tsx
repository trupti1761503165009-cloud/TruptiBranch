...
const HazardSubHazardStackChart: React.FC<ReportProps> = ({ data, width, height, title }) => {
    ...
    const renderChart = () => {
        ...
        chart.clear();
        const hazardTypes = unique(data.map(d => d.HazardType ?? "Unknown"));
        const subHazards = unique(data.map(d => d.HazardSubType ?? "Unknown"));

        const hazardTypeTotals = hazardTypes
            .map(ht => ({
                name: ht,
                total: data.filter(d => (d.HazardType ?? "Unknown") === ht).length
            }))
            .sort((a, b) => b.total - a.total);

        const sortedHazardTypeNames = hazardTypeTotals.map(h => h.name);
        ...
        const series = subHazards.map((sub, idx) => ({
            ...
            data: sortedHazardTypeNames.map(ht =>
                data.filter(
                    d =>
                        (d.HazardType ?? "Unknown") === ht &&
        ...
        chart.setOption({
            ...
            tooltip: {
                ...
                formatter: (params: any) => {
                    const hazardType = params[0]?.axisValue;
...
export default HazardSubHazardStackChart;
