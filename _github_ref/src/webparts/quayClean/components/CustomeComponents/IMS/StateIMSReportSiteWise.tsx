...
const StateIMSReportSiteWise: React.FC<Props> = ({ data }) => {
    ...
    const drawLevel0 = () => {
        ...
        chartInstance.current?.setOption({
            ...
            tooltip: {
                ...
                formatter: function (params: any) {
                    ...
                    const items = stateMap[state] || [];

                    const actionCounts = items.reduce((acc: any, item: any) => {
                    ...
                    return `<strong>State: ${state}</strong><br/><br/>${summary}`;
                }
            },
            series: [{
                type: 'bar',
        ...
        chartInstance.current?.on('click', (params: any) => {
            ...
            drawLevel1(params.name); // Pass state name to level 1
        });
    };

    const drawLevel1 = (state: string) => {
        setLevel(1);
        const entityMap: Record<string, ActivityItem[]> = {};

        const stateFiltered = filterData.filter(item => item.State === state);
            ...
            tooltip: {
                ...
                formatter: (params: any) => {
                    ...
                    const userDetailLines = actions
                        .map(item => `<strong>${item.UserName} (${item.ActionType})</strong><br/>${item.EntityName} at ${item.Modified}`)
                        .join('<br/><hr/>');
    ...
    const drawLevel2 = (entityType: string) => {
        setLevel(2);
        const filtered = filterData.filter(d => d.EntityType === entityType && d.State === selectedState);
        ...
        chartInstance.current?.setOption({
            ...
            tooltip: {
              ...
              <div style="margin-bottom: 5px;">
                ...
                <strong>Time:</strong> ${activity.Modified}
              </div>
              <hr/>
            `;
                    });
    ...
    const drawLevel3 = (entityType: string, siteName: string) => {
        ...
        // );
        const filtered = filterData.filter(
            d => d.EntityType === entityType && d.SiteName === siteName && d.State === selectedState
        );
        ...
        chartInstance.current?.setOption({
            title: {
                text: `Users in ${siteName} (${entityType})`,
                left: 'center'
            },
            xAxis: {
                type: 'category',
            ...
            tooltip: {
              ...
              <div style="margin-top: 5px;">
                ...
                <strong>Time:</strong> ${activity.Modified}<br/><hr/>
              </div>
            `;
                    });
            ...
            {!!filterData && filterData.length > 0 ?
                <div className="ims-Site-chart-card mt-3">
                    <div className="chart-header d-flex justify-content-between align-items-center dflex">
                        <div>
                            <Label className="chart-label">Activity by State Report</Label>
                        ...
                        {level > 0 && (
                            <PrimaryButton
                                text="Back"
                                onClick={() => {
                                    if (level === 3 && selectedEntityType && selectedSite) {
                                        drawLevel2(selectedEntityType);
                                    } else if (level === 2 && selectedEntityType) {
                                        drawLevel1(selectedState!); // Go back to entity level in same state
                                    } else if (level === 1) {
                                        drawLevel0(); // Go back to state level
                                    }
                                }}
...
export default StateIMSReportSiteWise;
