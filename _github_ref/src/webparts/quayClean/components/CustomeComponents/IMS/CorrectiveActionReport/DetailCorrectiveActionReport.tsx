...
import { DetailToolboxIncident } from "../ToolboxIncident/DetailToolboxIncident";
...
export const DetailCorrectiveActionReport: React.FC<IHelpDeskFormProps> = (props: IHelpDeskFormProps) => {
    ...
    const ViewIncidentReportId = React.useRef<any>(null);
    ...
    // const [selectedIncident, setselectedIncident] = React.useState(false);

    const _getCorrectiveActionReport = () => {
        setIsLoading(true);
        try {
            const select = ["ID,ReportDate,Location,Roles,Subject,IncidentReportId,ChairpersonId,Chairperson/Title,Chairperson/Name,SiteNameId,SiteName/Title,ReportId,Attendees,AttendeesEmailId,AttendeesEmail/Email,Attachments,AttachmentFiles,Created"];
            ...
            props.provider.getItemsByQuery(queryStringOptions).then((results: any[]) => {
                if (!!results) {
                    const UsersListData = results.map((data) => {
                        const fixImgURL = `${props.context.pageContext.web.serverRelativeUrl}/Lists/CorrectiveActionReport/Attachments/${data.ID}/`;
                        ...
                        return (
                            {
                                ID: data.ID,
                                Title: data.Title,
                                MeetingDate: !!data.ReportDate ? moment(data.ReportDate).format(DateFormat) : '',
                                SiteNameId: !!data.SiteNameId ? data.SiteNameId : '',
                                SiteName: !!data.SiteName ? data.SiteName.Title : '',
                                ReportId: !!data.ReportId ? data.ReportId : '',
                                IncidentReportId: !!data.IncidentReportId ? data.IncidentReportId : '',
                    ...
                    ViewIncidentReportId.current = UsersListData[0]?.IncidentReportId;
    ...
    const getCorrectiveActionReportMaster = () => {
        setIsLoading(true);
        try {
    ...
    const _getCorrectiveActionReportDetails = () => {
        setIsLoading(true);
        try {
    ...
    //Details Data
    const _getCorrectiveActionReportMasterData = () => {
        setIsLoading(true);
        try {
    ...
    const _getCorrectiveActionReportDetailsData = () => {
        setIsLoading(true);
        try {
    ...
    const onClickDownload = async (): Promise<void> => {
        setIsLoading(true);
        let fileName: string = SiteName + '-Corrective Action Report' + "-" + (!!ToolboxTalk[0]?.MeetingDate ? ToolboxTalk[0]?.MeetingDate : "");
        let fileblob: any = await generateAndSaveKendoPDF("DetailCorrectiveActionReportPDFCode", fileName, false, true);
        setIsLoading(false);
        setIsLoading(false);
    };
    ...
    const onClickSendEmail = async (): Promise<void> => {
        ...
        if (!isTitleEmpty && !isEmailEmpty && !isEmailInvalid) {
            // const fileName = `${SiteName} Corrective Action Report`;
            let fileName: string = SiteName + '-Corrective Action Report' + "-" + (!!ToolboxTalk[0]?.MeetingDate ? ToolboxTalk[0]?.MeetingDate : "");
            let fileblob: any = await generateAndSaveKendoPDF("DetailCorrectiveActionReportPDFCode", fileName, false);
            ...
            let insertData: any = {
                ...
                StateName: StateName,
                SiteName: SiteName,
                EmailType: "CorrectiveActionReport",
                Cleaner: CleanerName
            };
            props.provider.createItem(insertData, ListNames.SendEmailTempList).then((item: any) => {
                props.provider.uploadAttachmentToList(ListNames.SendEmailTempList, file, item.data.Id).then(() => {
                    ...
                    const logObj = {
                        ...
                        Details: `Send Email Corrective Action Report to ${sendToEmail}`,
    ...
    const _ToolboxTalkSignature = () => {
        setIsLoading(true);
        try {
            const select = ["ID,Signature,QuaycleanEmployeeId,QuaycleanEmployee/Email,QuaycleanEmployee/FirstName,QuaycleanEmployee/LastName,CorrectiveActionReportId,Created"];
            const expand = ["QuaycleanEmployee"];
            const queryStringOptions: IPnPQueryOptions = {
                select: select,
                expand: expand,
                filter: `CorrectiveActionReportId eq ${props?.siteMasterId}`,
        ...
        try {
            ...
            if (props?.componentProps?.qCStateId || data[0]?.QCStateId) {
                ...
                const queryStringOptions: IPnPQueryOptions = {
                    ...
                    filter: `Email eq '${currentUserRoleDetail?.emailId}' and EntityId eq '${props?.siteMasterId}' and SiteNameId eq '${orgSiteId}' and EntityType eq '${UserActionEntityTypeEnum.CorrectiveActionReport}' and ActionType eq 'Details View' and Created ge datetime'${todayDate}T00:00:00Z' and Created le datetime'${todayDate}T23:59:59Z'`
                };
    ...
    React.useEffect(() => {
        ...
        _getCorrectiveActionReport();
        _getCorrectiveActionReportMasterData();
        _getCorrectiveActionReportDetailsData();
    }, []);


    React.useEffect(() => {
        if (AllMasterData.length > 0)
            getCorrectiveActionReportMaster();
    }, [AllMasterData]);

    React.useEffect(() => {
        if (AllDetailData.length > 0)
            _getCorrectiveActionReportDetails();
    }, [AllDetailData]);
    ...
    const onClickClose = () => {
        ...
        //         currentComponentName: ComponentNameEnum.AddNewSite, qCStateId: props?.componentProps?.qCStateId, view: props.componentProps.viewType, dataObj: props.componentProps.dataObj, breadCrumItems: breadCrumItems, siteMasterId: props.originalSiteMasterId, isShowDetailOnly: true, siteName: props.componentProps.siteName, qCState: props.componentProps.qCState, pivotName: "IMSKey", subpivotName: "CorrectiveActionReport",
        ...
        if (isSiteLevelComponent) {
            ...
            const breadCrumItems = removeElementOfBreadCrum(props.breadCrumItems);
            manageComponentView({ currentComponentName: ComponentNameEnum.Quaysafe, qCStateId: props?.componentProps?.qCStateId, view: props.componentProps.viewType, breadCrumItems: breadCrumItems, subpivotName: "CorrectiveActionReport" });
        }
    };

    const onClickPanel = () => {
        setShowModal(true);
        // setselectedIncident(true);
    };
    ...
    return <>
        ...
        <div className="correctiveActionReport-Detail">
                            ...
                            <div id="ToolboxTalk" className="asset-card-2-header-jcc margin-bot-80">
                                ...
                                <div className="" >
                                    <div className="boxCard">
                                        <div id="DetailCorrectiveActionReportPDFCode">
                                            ...
                                            <div className="meeting-id-cls"><div className="td-toolbox-talk blue-text dflex justify-content-end meeting-spc-cls"><div className="toggle-className">Corrective Action Report ID: {ToolboxTalk[0]?.ReportId}</div></div></div>
                                            <div className="asset-card-2-header-jcc">
                                                <div className="table-toolbox-talk mt-3">
                                                    ...
                                                    <div className="row">
                                                        <div className="td-toolbox-talk-detail"><b>Role:</b></div>
                                                        <div className="td-toolbox-talk-detail">{ToolboxTalk[0]?.Roles}</div>
                                                    </div>
                                                    <div className="row">
                                                    ...
                                                    <div className="row">
                                                        <div className="td-toolbox-talk-detail">
                                                            <b>Incident report link:</b>
                                                        </div>
                                                        <div className="td-toolbox-talk-detail">
                                                            {ViewIncidentReportId.current
                                                                ? (
                                                                    <span
                                                ...
                                                <div>
                                                    <div>
                                                        {ToolboxTalkData.map((item: any) => {
                                                            ...
                                                            if (item.IsShow) {
                                                                const details = ToolboxTalkDetailsData.filter((detail: any) => detail.CorrectiveActionReportMasterId === item.ID);
                                                                    ...
                                                                    <div key={item.ID} className="">
                                                                        ...
                                                                        {item.SectionType === "RichTextbox" && (
                                                                            <div className="keep-together">
                                                                                {/* <div className="pdf-lbl-talk mt-1"><b>{item.CommentTitle ? item.CommentTitle : "Comments"}</b></div> */}
        ...
        <Panel
            ...
            customWidth="860px"
            headerText="Incident Details" // You can use PanelType.medium or custom width
        >
            {(
                ...
                <DetailToolboxIncident
                    ...
                    siteMasterId={ViewIncidentReportId.current}
