...
export const StateReport = (props: IAssociateChemicalProps) => {
    ...
    React.useEffect(() => {
        ...
        const drawLevel0 = () => {
            ...
            chartInstance.current?.setOption({
                title: { text: 'Activity by State', left: 'center' },
                xAxis: {
                    type: 'category',
                    data: stateNames,
        ...
        const drawLevel1 = () => {
            ...
            chartInstance.current?.setOption({
                title: { text: `Activity by Site - ${selectedState}`, left: 'center' },
        ...
        const drawLevel2 = () => {
            ...
            chartInstance.current?.setOption({
                title: { text: `User Activity - ${selectedSite}`, left: 'center' },
        ...
        const drawLevel3 = () => {
            ...
            chartInstance.current?.setOption({
                title: { text: `Activity Timeline - ${selectedUser}`, left: 'center' },
                            ...
                            <div className="mt-6 bg-white p-4 rounded shadow mt-5">
                                <div className="chart-div-table">
                                    <div className="chart-div-header">
                                        <div className="chart-div-cell">State</div>
                                    </div>

                                    {Object.entries(groupedByState).map(([state, sites]: any, stateIndex) => (
                                        <div key={state}>
                                            <div
                                                className={`chart-div-row ${stateIndex % 2 !== 0 ? 'white-bg' : ''}`}
                                                onClick={() =>
                                                ...
                                                <div className="chart-div-cell">
                                                    <div className="dflex">
                                                        <FontAwesomeIcon
                                                            ...
                                                            icon={expandedStates[state] ? 'caret-down' : 'caret-right'}
                                                        />
                                                        {state}
                                                    </div>
                                                </div>
                                            ...
                                            {expandedStates[state] && (
                                                <div style={{ paddingLeft: '20px' }}>
                                                    ...
                                                    {Object.entries(sites).map(([site, info]: any, siteIndex) => {
                                                        const actionsByDate = info.items.reduce((acc: any, item: any) => {
                                                            const date = item.Modified.split(' ')[0];
                                                            if (!acc[date]) acc[date] = [];
                                                            acc[date].push(item);
                                                            return acc;
                                                        }, {});
                                                        return (
                                                            <div key={site}>
                                                                <div
                                                                    className={`chart-div-row ${siteIndex % 2 !== 0 ? 'white-bg' : ''}`}
                                                                    onClick={() =>
