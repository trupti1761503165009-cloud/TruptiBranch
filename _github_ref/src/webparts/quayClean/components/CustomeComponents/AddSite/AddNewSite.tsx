...
import { DialogFooter, Dropdown, FocusTrapZone, Icon, IDropdownOption, IPersonaProps, IPivotItemProps, Label, Layer, Overlay, Popup, Toggle } from "office-ui-fabric-react";
...
import { ComponentNameEnum, getExternalUrl, ListNames, UserActionEntityTypeEnum, UserActivityActionTypeEnum, viewDetailStickHeaders } from "../../../../../Common/Enum/ComponentNameEnum";
import { IErrorLog } from "../../../../../Interfaces/IErrorLog";
import { canShowSiteActionButtons, encryptValue, encryptWasteValue, getCAMLQueryFilterExpression, getPeopleDifferences, getSiteGroupsPermission, getSiteMasterItems, getStateMasterItems2, logGenerator, mapSingleValue, onBreadcrumbItemClicked, saveThumbNailImage, UserActivityLog } from "../../../../../Common/Util";
...
import SectionOrderManager, { ISectionOrderManagerRef } from "../../CommonComponents/DragDrop/DragSiteDetailsQRMenu";
...
export const AddNewSite = (props: IAddNewSiteProps) => {
    ...
    const [SiteDetailsQRMenu, setSiteDetailsQRMenu] = React.useState<any>("");
    const [UpdateMenuFunction, setUpdateMenuFunction] = React.useState(false);
    ...
    const _onClickViewPanel = () => {
        setIsDetails(true);
    };

    const sectionRef = React.useRef<ISectionOrderManagerRef>(null);

    const saveConfig = async () => {
        const toastId = toastService.loading('Loading...');
        const toastMessage = 'Site details QR menu saved successfully!';
        setIsLoading(true);
        const json = sectionRef?.current?.getJson();
        const jsonString = JSON?.stringify(json);
        await provider.updateItemWithPnP({ SiteDetailsQRMenu: jsonString }, ListNames.SitesMaster, Number(props?.siteMasterId));
        toastService.updateLoadingWithSuccess(toastId, toastMessage);
        setIsDetails(false);
        setUpdateMenuFunction(prev => !prev);
        setSiteDetailsQRMenu("");
    ...
    const onClickClose = (): void => {
        ...
        if (props.isZoneAddNewSite || props.isZoneEdit) {
    ...
    const getSiteMasteItemsforQRMenu = () => {
        if (!!siteMasterId) {
            const queryOptions: IPnPQueryOptions = {
                listName: ListNames.SitesMaster,
                select: ['Id,Title,SiteDetailsQRMenu'],
                id: !!siteMasterId ? siteMasterId : 0
            };
            return provider.getByItemByIDQuery(queryOptions);
        }
    ...
    React.useEffect(() => {
        if (!!props.isZoneEdit && props.isZoneEdit) {
            onclickEdit();
        }
    ...
    const onclickEdit = async () => {
        ...
        try {
            ...
            setNewFromObj((prevState: any) => ({
                ...
                SiteCategoryId: siteMasterItems?.SiteCategory?.Id,
                Category: siteMasterItems?.SiteCategory?.Title,
    ...
    React.useEffect(() => {
        const fetchforQRMenu = async () => {
            try {
                const siteMasterItemsforQRMenu = await getSiteMasteItemsforQRMenu();
                const jsonObject = JSON.parse(siteMasterItemsforQRMenu?.SiteDetailsQRMenu);
                setSiteDetailsQRMenu(jsonObject);
            } catch (error) {
                console.error("Failed to fetch site details for qr menu:", error);
            }
        };
        fetchforQRMenu();
    }, [UpdateMenuFunction]);
    ...
    const onRenderFooterContent = () => {
        ...
        return <div className="dflex">
            <div className="margin-sm-add-btn-panel">
                <PrimaryButton className={"btn btn-primary"} onClick={saveConfig} text="Save" />
            </div>
            <div className="margin-sm-add-btn-panel">
                <PrimaryButton className="btn btn-danger" onClick={onClickClosePanel} text="Close" />
            </div>
            {/* <div className="margin-sm-add-btn-panel">
    ...
    return <>
                                        ...
                                        <div className="ms-Grid">
                                            <div className="ms-Grid-row">
                                                    ...
                                                    {(state.isaddNewSite) ?
                                                        <div className="radioCheckGroup">
                                                            ...
                                                            <div className="flex-center">
                                                                ...
                                                                {IsSubLocation &&
                                                                    <TooltipHost content="Add location" id={tooltipId}>
                                                                        <FontAwesomeIcon
                                                                            icon="plus"
                                                ...
                                                <div className="ms-Grid-row">
                                                    ...
                                                    <div className="ms-Grid-col ms-sm12 ms-md4 ms-lg4 mb4 mt-3">

                                                        {(props.loginUserRoleDetails.isAdmin ||
                                                            props.loginUserRoleDetails.isStateManager ||
                                                            props.loginUserRoleDetails?.siteManagerItem?.some(
                                                                r =>
                                                                    r.Id === props.siteMasterId &&
                                                                    r.SiteManagerId?.indexOf(props.loginUserRoleDetails.Id) > -1
                                                            )) && (
                                                                <TooltipHost content="Supervisor Access">
                                                                    <PrimaryButton
                                                        ...
                                                        {(props.loginUserRoleDetails.isAdmin ||
                                                            ...
                                                            props.loginUserRoleDetails?.siteManagerItem?.some(
                                                                ...
                                                                <TooltipHost content="Site Details QR Menu">
                                                                    <PrimaryButton
                                                                        iconProps={{ iconName: "Settings" }}
                                                                        className="btn btn-primary ml-10"
                                                                        onClick={_onClickViewPanel}
                                                                        text="Site Details QR Menu"
                                                                    />
                                                                </TooltipHost>
                                                            )}
                                                    </div>
                                                        ...
                                                        <div className="ms-Grid-col ms-sm12 ms-md4 ms-lg4 mb4">
                                                            <div className="formControl mt-3">
                                                                <Label className="labelform">
                                                                    {WasteReportViewFields.BreakDownBy}
                                                ...
                                                {/* <div className="ms-Grid-col ms-sm6 ms-md4 ms-lg4 justify-content-end" style={{ marginTop: "24px", minHeight: "76px" }}>
                                    ...
                                    >
                                        ...
                                        >
                                            <ManageSitesCrud
                                                ...
                                                qrCodeSrc={qrCodeSrc}
                                                isCrudShow={canShowAddEditButton}
        ...
        {isDetails &&
            // <Panel
            //     isOpen={isDetails}
            //     onDismiss={() => onClickClosePanel()}
            //     isBlocking={true}
            //     type={PanelType.custom}
            //     headerText="Site Details QR Menu"
            ...
            <Panel
                ...
                onDismiss={onClickClosePanel}
                isBlocking={false}
                isLightDismiss               // ✅ allows outside click
                type={PanelType.custom}
                layerProps={{
                    styles: {
                        root: {
                            zIndex: 1075    // LOWER panel zIndex
                        }
                    }
                }}
            ...
            >
                ...
                <div>
                    <div className="donlyFlex justifyContentBetween mt-2">
                        <SectionOrderManager
                            ref={sectionRef}
                            context={props.context}
                            provider={provider}
                            SiteId={props?.siteMasterId}
                            // resetFlag={resetFlag}
                            SiteDetailsQRMenu={SiteDetailsQRMenu}
                        />
                    </div>
                </div>
